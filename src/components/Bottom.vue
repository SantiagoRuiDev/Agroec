<template>
  <ion-footer class="shadow rounded-t-xl relative">
    <!--<NotificationButton></NotificationButton>-->
    <ion-toolbar class="shadow rounded-t-xl z-0">
      <div class="flex justify-evenly w-full h-12 items-center">
        <RouterLink to="/app/home">
          <svg viewBox="0 0 19 17" xmlns="http://www.w3.org/2000/svg" class="nav-item" fill="#37474F" stroke-width="0">
            <path fill-rule="evenodd" clip-rule="evenodd"
              d="M8.19785 10.655V15.655C8.19785 16.205 7.74785 16.655 7.19785 16.655H3.19785C2.64785 16.655 2.19785 16.205 2.19785 15.655V8.655H0.497852C0.037852 8.655 -0.172148 8.085 0.167852 7.785L8.52785 0.255C8.90785 -0.085 9.48785 -0.085 9.86785 0.255L18.2279 7.785C18.5779 8.085 18.3579 8.655 17.8979 8.655H16.1979V15.655C16.1979 16.205 15.7479 16.655 15.1979 16.655H11.1979C10.6479 16.655 10.1979 16.205 10.1979 15.655V10.655H8.19785ZM14.1979 7.06768C14.1979 6.92594 14.1377 6.79086 14.0323 6.69603L9.53233 2.64603C9.34218 2.4749 9.05352 2.4749 8.86337 2.64603L4.36337 6.69603C4.25801 6.79086 4.19785 6.92594 4.19785 7.06768V14.155C4.19785 14.4311 4.42171 14.655 4.69785 14.655H5.69785C5.97399 14.655 6.19785 14.4311 6.19785 14.155V8.655H12.1979V14.155C12.1979 14.4311 12.4217 14.655 12.6979 14.655H13.6979C13.974 14.655 14.1979 14.4311 14.1979 14.155V7.06768Z" />
          </svg>
        </RouterLink>
        <RouterLink to="/orders">
          <svg fill="#37474F" class="nav-item" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
            xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 470 470" xml:space="preserve" stroke="#37474F"
            stroke-width="12.22">
            <g id="SVGRepo_bgCarrier" stroke-width="0" />

            <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round" stroke="#CCCCCC"
              stroke-width="2.82" />

            <g id="SVGRepo_iconCarrier">
              <g>
                <path d="M290,102.736c4.411,0,8-3.589,8-8s-3.589-8-8-8s-8,3.589-8,8S285.589,102.736,290,102.736z" />
                <path d="M270,72.736c4.411,0,8-3.589,8-8s-3.589-8-8-8s-8,3.589-8,8S265.589,72.736,270,72.736z" />
                <path d="M190,94.736c4.411,0,8-3.589,8-8s-3.589-8-8-8s-8,3.589-8,8S185.589,94.736,190,94.736z" />
                <path
                  d="M406.322,409.316l-10.934-10.934l-6.57-188.861c11.321-3.342,19.608-13.826,19.608-26.215v-38.138 c0-15.072-12.262-27.334-27.334-27.334h-18.28C300.124,25.442,272.378,0,235,0c-37.378,0-65.124,25.442-127.813,117.835h-18.28 c-15.072,0-27.334,12.262-27.334,27.334v38.138c0,12.389,8.287,22.873,19.608,26.215l-6.57,188.861l-10.935,10.934 c-13.034,13.035-13.034,34.244,0,47.278c6.314,6.314,14.709,9.792,23.64,9.792c8.93,0,17.324-3.477,23.639-9.792l1.647-1.647 C146.97,464.267,193.347,470,235,470s88.03-5.733,122.397-15.053l1.647,1.647c6.314,6.314,14.709,9.792,23.639,9.792 c8.931,0,17.325-3.477,23.64-9.792C419.357,443.56,419.357,422.351,406.322,409.316z M369.652,445.988l-4.748-4.748 c-1.931-1.93-4.758-2.66-7.384-1.903C324.048,448.999,277.101,455,235,455s-89.048-6.001-122.521-15.663 c-2.629-0.757-5.452-0.027-7.384,1.903l-4.748,4.748c-3.48,3.481-8.108,5.398-13.031,5.398c-4.924,0-9.552-1.917-13.032-5.398 c-7.187-7.187-7.187-18.879-0.001-26.065L87.307,406.9c1.343-1.343,2.127-3.145,2.192-5.042l6.651-191.216h22.757 c4.143,0,7.5-3.358,7.5-7.5c0-4.142-3.357-7.5-7.5-7.5h-30c-6.801,0-12.334-5.533-12.334-12.334v-38.138 c0-6.801,5.533-12.334,12.334-12.334h292.186c6.801,0,12.334,5.533,12.334,12.334v38.138c0,6.801-5.533,12.334-12.334,12.334 H148.907c-4.143,0-7.5,3.358-7.5,7.5c0,4.142,3.357,7.5,7.5,7.5H373.85l6.651,191.216c0.065,1.898,0.85,3.7,2.192,5.042 l13.022,13.023c7.187,7.186,7.187,18.879,0,26.065c-3.48,3.481-8.108,5.398-13.032,5.398 C377.761,451.386,373.133,449.469,369.652,445.988z M235,15c30.151,0,54.572,22.467,109.659,102.835H125.341 C180.428,37.467,204.849,15,235,15z" />
                <path
                  d="M331.563,254.125l-28.604,29.458l-28.604-29.458c-1.412-1.455-3.354-2.275-5.381-2.275s-3.969,0.821-5.381,2.276 l-28.6,29.457l-28.598-29.457c-1.412-1.455-3.354-2.276-5.381-2.276s-3.969,0.821-5.381,2.276l-28.599,29.457l-28.598-29.457 c-2.885-2.972-7.631-3.042-10.605-0.157c-2.972,2.885-3.042,7.634-0.156,10.605l33.979,35c1.412,1.455,3.354,2.276,5.381,2.276 s3.969-0.821,5.381-2.276l28.599-29.457l28.598,29.457c1.412,1.455,3.354,2.276,5.381,2.276s3.969-0.821,5.381-2.276l28.6-29.458 l28.604,29.458c1.412,1.455,3.354,2.275,5.381,2.275s3.969-0.821,5.381-2.275l33.985-35c2.886-2.972,2.815-7.72-0.156-10.605 C339.197,251.083,334.448,251.152,331.563,254.125z" />
              </g>
            </g>
          </svg>
        </RouterLink>
        <RouterLink to="/app/licitaciones/filter">
          <svg xmlns="http://www.w3.org/2000/svg" fill="#37474F" class="nav-item" viewBox="0 -960 960 960">
            <path
              d="M475-160q4 0 8-2t6-4l328-328q12-12 17.5-27t5.5-30q0-16-5.5-30.5T817-607L647-777q-11-12-25.5-17.5T591-800q-15 0-30 5.5T534-777l-11 11 74 75q15 14 22 32t7 38q0 42-28.5 70.5T527-522q-20 0-38.5-7T456-550l-75-74-175 175q-3 3-4.5 6.5T200-435q0 8 6 14.5t14 6.5q4 0 8-2t6-4l136-136 56 56-135 136q-3 3-4.5 6.5T285-350q0 8 6 14t14 6q4 0 8-2t6-4l136-135 56 56-135 136q-3 2-4.5 6t-1.5 8q0 8 6 14t14 6q4 0 7.5-1.5t6.5-4.5l136-135 56 56-136 136q-3 3-4.5 6.5T454-180q0 8 6.5 14t14.5 6Zm-1 80q-37 0-65.5-24.5T375-166q-34-5-57-28t-28-57q-34-5-56.5-28.5T206-336q-38-5-62-33t-24-66q0-20 7.5-38.5T149-506l232-231 131 131q2 3 6 4.5t8 1.5q9 0 15-5.5t6-14.5q0-4-1.5-8t-4.5-6L398-777q-11-12-25.5-17.5T342-800q-15 0-30 5.5T285-777L144-635q-9 9-15 21t-8 24q-2 12 0 24.5t8 23.5l-58 58q-17-23-25-50.5T40-590q2-28 14-54.5T87-692l141-141q24-23 53.5-35t60.5-12q31 0 60.5 12t52.5 35l11 11 11-11q24-23 53.5-35t60.5-12q31 0 60.5 12t52.5 35l169 169q23 23 35 53t12 61q0 31-12 60.5T873-437L545-110q-14 14-32.5 22T474-80Zm-99-560Z" />
          </svg>
        </RouterLink>
        <RouterLink to="/app/licitar">
          <svg xmlns="http://www.w3.org/2000/svg" fill="#37474F" class="nav-item" viewBox="0 -960 960 960">
            <path d="M440-440H200v-80h240v-240h80v240h240v80H520v240h-80v-240Z" />
          </svg>
        </RouterLink>
      </div>
    </ion-toolbar>
  </ion-footer>
</template>


<script allowjs>
import { IonFooter, IonToolbar } from "@ionic/vue"
import NotificationButton from "./NotificationButton.vue";
export default {
  components: {
    IonFooter,
    IonToolbar,
    NotificationButton
  },
  methods: {
    showMenu() {
      this.$emit("toggle");
    },
  }
}
</script>

<style scoped>
.nav-item {
  padding: 10px;
  border-radius: 50%;
  height: 45px;
  width: 43px;
  margin: 0px auto;
}

@media screen and (min-width: 768px) {
  .nav-item {
    border-radius: 47%;
    height: 45px;
    width: 45px;
  }
}

.nav-item:hover {
  background-color: #a8cf7b;
  fill: #ffffff;
  stroke: #ffffff;
}
</style>
