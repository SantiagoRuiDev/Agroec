  ...
  <div class="content w-11/12 mx-auto grid my-3 gap-3" v-if="!condiciones && order">
    ...
    <div class="deliver-status flex gap-4 mt-3 w-full mx-auto">
      ...
      <div class="deliver-cards grid gap-3 w-full">
        ...
        <div class="bg-yellow-100 text-left px-4 py-3 rounded-md grid gap-1" v-else>
          <h2 class="text-md text-gray-700">{{ order.modo_pago }} de {{ formatToUSD(order.precio *
            order.condicion_cantidad) }}
          </h2>
    ...
    <div class="deliver-price grid gap-3" v-if="order">
      ...
      <div class="grid grid-cols-2">
        ...
        <p class="text-sm text-gray-700 text-right">{{ formatToUSD(order.precio) }}</p>
      </div>
      <div class="grid grid-cols-2">
      ...
      <div class="grid grid-cols-2">
        ...
        <p class="text-sm text-gray-800 text-right font-bold">{{ formatToUSD(order.condicion_cantidad * order.precio) }}
        </p>
      </div>
    </div>
  </div>
      ...
      <div class="grid w-full gap-2 pb-3">
        ...
        <div class="gap-1 mx-auto grid">
          ...
          <h1 class="text-center text-gray-700 font-bold text-xl">
            Total <span class="text-lime-500 font-bold"></span>{{ formatToUSD((order.precio * order.condicion_cantidad)
              *
          ...
          <div class="form-input grid gap-1" v-if="warrantyPaymentMethod == 'TD/TC' && cards.length > 0">
            ...
            <input type="text" id="documento" @change="verifyDocument" v-model="documento" placeholder="El mismo que el de la tarjeta"
              class="bg-transparent p-2 h-12 border-2 rounded-md w-auto text-gray-700" />
          </div>
